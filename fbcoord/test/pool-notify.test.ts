...
describe('PoolDurableObject notifications', () => {
  ...
  it('schedules stale alarms and emits timeout notifications without follow-up traffic', async () => {
    vi.useFakeTimers();
    ...
    const internals = pool as unknown as {
      ...
      persistIfNeeded(changed: boolean): Promise<void>;
      syncStaleAlarm(): Promise<void>;
      state: {
        registerConnection(nodeId: string, connection: WebSocket): { rosterChanged: boolean };
      };
    };
    await internals.ensureLoaded();
    const socket = new FakeSocket();
    const result = internals.state.registerConnection('node-1', socket as unknown as WebSocket);
    await internals.persistIfNeeded(result.rosterChanged);
    await internals.syncStaleAlarm();
    await flush();

    expect(await storage.getAlarm()).toBe(Date.now() + 30_000);

    vi.setSystemTime(new Date('2026-04-10T00:00:31Z'));
    await pool.alarm();
    await flush();
