...
func (s *Store) queryWithoutCIDR(params QueryParams) (QueryResult, error) {
	...
	query := `SELECT id, ip, asn, as_org, country, protocol, upstream, port, bytes_up, bytes_down, duration_ms, recorded_at FROM ip_log` + where + ` ORDER BY recorded_at DESC LIMIT ? OFFSET ?`
...
func (s *Store) queryWithCIDR(params QueryParams) (QueryResult, error) {
	...
	query := `SELECT id, ip, asn, as_org, country, protocol, upstream, port, bytes_up, bytes_down, duration_ms, recorded_at FROM ip_log` + where + ` ORDER BY recorded_at DESC`
	rows, err := s.readDB.Query(query, args...)
	if err != nil {
		return QueryResult{}, err
	}
