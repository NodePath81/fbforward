...
func TestHTMLHasNoInlineOrExternalResources(t *testing.T) {
	...
	for _, id := range []string{"login-error", "instance-summary", "upstream-rows", "route-rows", "flow-rows", "context-head", "context-rows", "context-prev", "context-next", "audit-form", "audit-query", "audit-view-toggle", "audit-raw", "firewall-status"} {
...
func TestClientSecurityInvariants(t *testing.T) {
	...
	for _, required := range []string{"requestJSON('/identity')", "logout('invalid token')", "showLoginError('')", "if (state.identityLoaded) return", "state.flows", "renderFlowTable()", "ListFlowContextTags", "ListFlowContextActions", "contextView", "contextGeneration", "generation === state.contextGeneration", "page: 'audit'", "history.replaceState", "refreshPending", "auditPending", "auditGeneration", "performance.now()", "requestPage", "QueryAudit", "top tags", "audit-query", "audit-view-toggle", "key === 'Escape'", "pollingIntervals = { status: 5000, flows: 2000 }", "setAttribute('aria-current', 'page')", "SetRouteOverride", "ClearRouteOverride"} {
	...
	for _, required := range []string{`nav button[aria-current="page"]`, ".sr-only", ".context-scroll", ".context-tabs", "width: max-content", "min-width: 100%"} {
