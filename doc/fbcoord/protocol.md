...
fbcoord. For deployment, UI usage, and troubleshooting, see the
[fbcoord user guide](user-guide.md). For admin HTTP endpoint details, see the
[fbcoord API reference](api.md).
...
10. On disconnect or stale eviction without accepted teardown, the node moves
    to `aborted`.
...
The admin HTTP surface for this state is `GET /api/state`, documented in the
[fbcoord API reference](api.md#api-state). Important semantic points:

- `node_count` counts only `online` nodes
- `counts` includes `online`, `offline`, `aborted`, and `never_seen`
