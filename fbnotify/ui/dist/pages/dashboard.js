...
function buildTargetForm(editingTarget) {
...
function buildRouteForm(editingRoute, targets) {
...
export function renderDashboardPage(data, handlers) {
  ...
  `;
    ...
    const refreshButton = shell.querySelector('#refresh-page');
    const targetForm = shell.querySelector('#target-form');
    const routeForm = shell.querySelector('#route-form');
    const nodeTokenForm = shell.querySelector('#node-token-form');
    const rotateGeneratedForm = shell.querySelector('#rotate-generated-form');
    const rotateCustomForm = shell.querySelector('#rotate-custom-form');
    const testSendForm = shell.querySelector('#test-send-form');
    const clearCaptureButton = shell.querySelector('#clear-capture');
    const targetConfigHost = shell.querySelector('#target-config');
    const targetTypeSelect = shell.querySelector('#target-type');
    const testResult = shell.querySelector('#test-send-result');
    ...
    targetForm.addEventListener('submit', event => {
        ...
        const name = shell.querySelector('#target-name')?.value.trim() ?? '';
        const selectedType = editingTarget?.type ?? targetTypeSelect.value;
        const payload = {
            name,
        ...
        if (selectedType === 'webhook') {
            const url = shell.querySelector('#target-url')?.value.trim() ?? '';
            payload.config = url ? { url } : {};
        }
        else if (selectedType === 'pushover') {
            const apiToken = shell.querySelector('#target-api-token')?.value.trim() ?? '';
            const userKey = shell.querySelector('#target-user-key')?.value.trim() ?? '';
            const device = shell.querySelector('#target-device')?.value.trim() ?? '';
            payload.config = {
    ...
    shell.querySelector('#cancel-target-edit')?.addEventListener('click', () => {
        handlers.onStartEditTarget(null);
    });
    for (const button of shell.querySelectorAll('.target-edit')) {
        button.addEventListener('click', () => {
            handlers.onStartEditTarget(button.dataset.targetId ?? null);
        });
    }
    for (const button of shell.querySelectorAll('.target-delete')) {
        button.addEventListener('click', () => {
    ...
    routeForm.addEventListener('submit', event => {
        event.preventDefault();
        const targetIds = Array.from(shell.querySelectorAll('input[name="route-target"]:checked'))
            .map(input => input.value);
        const payload = {
            name: shell.querySelector('#route-name')?.value.trim() ?? '',
            event_name: shell.querySelector('#route-event')?.value.trim() ?? '',
            source_service: shell.querySelector('#route-service')?.value.trim() ?? '',
            target_ids: targetIds
        };
        void handlers.onSaveRoute(payload, editingRoute?.id ?? null);
    });
    shell.querySelector('#cancel-route-edit')?.addEventListener('click', () => {
        handlers.onStartEditRoute(null);
    });
    for (const button of shell.querySelectorAll('.route-edit')) {
        button.addEventListener('click', () => {
            handlers.onStartEditRoute(button.dataset.routeId ?? null);
        });
    }
    for (const button of shell.querySelectorAll('.route-delete')) {
        button.addEventListener('click', () => {
    ...
    nodeTokenForm.addEventListener('submit', event => {
        event.preventDefault();
        const sourceService = shell.querySelector('#source-service')?.value.trim() ?? '';
        const sourceInstance = shell.querySelector('#source-instance')?.value.trim() ?? '';
        void handlers.onCreateNodeToken(sourceService, sourceInstance);
    });
    for (const button of shell.querySelectorAll('.node-token-delete')) {
        button.addEventListener('click', () => {
    ...
    rotateGeneratedForm.addEventListener('submit', event => {
        event.preventDefault();
        const currentToken = shell.querySelector('#current-operator-token')?.value.trim() ?? '';
        void handlers.onRotateGenerate(currentToken);
    });
    rotateCustomForm.addEventListener('submit', event => {
        event.preventDefault();
        const currentToken = shell.querySelector('#current-operator-token')?.value.trim() ?? '';
        const replacement = shell.querySelector('#custom-operator-token')?.value.trim() ?? '';
        void handlers.onRotateCustom(currentToken, replacement);
    });
    testSendForm.addEventListener('submit', event => {
        event.preventDefault();
        const raw = shell.querySelector('#test-event-json')?.value ?? '';
        let parsed;
        try {
        ...
        const selectedTargets = Array.from(shell.querySelectorAll('input[name="test-target"]:checked')).map(input => input.value);
