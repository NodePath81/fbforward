function panel(content: string): HTMLElement {
...
export function renderLoginPage(params: { onSubmit: (token: string) => Promise<void> }): HTMLElement {
  const page = panel(`
    <section class="panel">
  ...
  const input = page.querySelector<HTMLInputElement>('#operator-token');
  if (!form || !input) {
